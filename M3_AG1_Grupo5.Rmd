...
Se presenta el summary de todas las variables en donde podemos ver que existe un valor faltante o NA en Internet_por_num, mientras que el resto de las variables presenta todos sus registros. Por otra parte, se observa que Internet_por_num y Facebook_por_num están en escala de porcentajes. 

# 2. Búsqueda de outliers

```{r}
datos %>% ggplot(aes(x = Facebook_por_num)) + 
...
datos %>% ggplot(aes(x = Facebook_por_num)) + 
  ...
  theme(axis.ticks.y = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5))
```
Al realizar el análisis por año vemos que en el año 2010 se presenta un outlier por un valor inusualmente alto en comparación a la distribución del resto de los datos, mientras que en el año 2018 esta situación se invierte ya que se encuentran varios outliers pero esta vez por valores inusualmente bajos.
